...
def generate_text_3434(project_name, s3_usage, s3_settings):
    ...
    if s3_usage:
        ...
        s3c_interim.upload_file_to_s3(
            filepath=folder_text_3434 + r"/text_3434.csv", s3_prefix=project_prefix_text3434, s3_key="text_3434.csv"
        )
...
def main():
    ...
    try:
        ...
        destination_saved_models_relevance = (
            project_model_dir + r"/RELEVANCE/Text" + r"/" + relevance_training_output_model_name
        )
        destination_saved_models_inference = (
            project_model_dir + r"/KPI_EXTRACTION/Text" + r"/" + kpi_inference_training_output_model_name
        )
        ...
        end_to_end_response = run_router(
            ext_port, infer_port, project_name, ext_ip, infer_ip, s3_usage, s3c_main, s3c_interim
        )
        ...
        if end_to_end_response:
            if project_settings["extraction"]["store_extractions"]:
                ...
                if s3_usage:
                    ...
                    end_to_end_response = copy_file_without_overwrite(
                        source_extraction_data, destination_extraction_data
                    )
