...
@pytest.fixture
def prerequisites_run_router(
    prerequisites_convert_xls_to_csv, prerequisites_generate_text
    ...
    with (
        requests_mock.Mocker() as mocked_server,
...
def test_run_router_extraction_liveness_up(
    prerequisites_run_router: requests_mock.mocker.Mocker,
    status_code: int,
...
def test_run_router_kpi_training(
    ...
    with patch("train_on_pdf.generate_text_3434", mocked_generate_text):
        mocked_server.get(
            f"http://{inference_ip}:{inference_port}/infer_relevance", status_code=status_code_infer_relevance
        )
...
def test_run_router_successful_run(
    prerequisites_run_router: requests_mock.mocker.Mocker, infer_relevance: bool, train_kpi: bool
):
