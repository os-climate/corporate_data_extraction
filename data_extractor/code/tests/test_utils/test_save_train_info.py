...
def prerequisites_save_train_info(path_folder_root_testing: Path, path_folder_temporary: Path) -> Path:
    ...
    mocked_project_settings = {
        "train_relevance": {"output_model_name": "TEST"},
        "train_kpi": {"output_model_name": "TEST"},
        "s3_settings": {"prefix": "corporate_data_extraction_projects"},
    }
...
def test_save_train_info_entries(prerequisites_save_train_info: Path):
    ...
    expected_keys = ["project_name", "train_settings", "pdfs_used", "annotations", "kpis"]
