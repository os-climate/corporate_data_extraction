...
def prerequisite_train_on_pdf_try_run(
    request: FixtureRequest, path_folder_root_testing: Path, path_folder_temporary: Path, prerequisite_running
) -> None:
    ...
    mocked_project_settings = {
        ...
        "train_relevance": {"output_model_name": "test", "train": False},
        "train_kpi": {"output_model_name": "test", "train": False},
        "extraction": {"use_extractions": False, "store_extractions": False},
    }
...
def test_train_on_pdf_wrong_input_project_name(
    project_name: typing.Union[str, None], capsys: typing.Generator[CaptureFixture[str], None, None]
):
...
@pytest.mark.parametrize("s3_usage", [None, "X"])
def test_train_on_pdf_wrong_input_s3(
    s3_usage: typing.Union[str, None], capsys: typing.Generator[CaptureFixture[str], None, None]
):
...
@pytest.mark.parametrize("s3_usage", ["Y", "N"])
def test_train_on_pdf_correct_input_s3_usage(s3_usage: typing.Union[str, None]):
    ...
    with (
        ...
        patch("train_on_pdf.input", Mock()) as mocked_input,
        patch(
            "train_on_pdf.create_directory", side_effect=lambda *args: Path(args[0]).mkdir(parents=True, exist_ok=True)
        ),
...
@pytest.mark.parametrize("prerequisite_train_on_pdf_try_run", [("train_relevance", "train", True)], indirect=True)
...
@pytest.mark.parametrize("prerequisite_train_on_pdf_try_run", [("train_kpi", "train", True)], indirect=True)
...
@pytest.mark.parametrize(
    "prerequisite_train_on_pdf_try_run", [("extraction", "store_extractions", True)], indirect=True
)
def test_train_on_pdf_e2e_store_extractions(
    path_folder_temporary: Path, capsys: typing.Generator[CaptureFixture[str], None, None]
):
...
@pytest.mark.parametrize(
    "prerequisite_train_on_pdf_try_run", [("general", "delete_interim_files", True)], indirect=True
)
