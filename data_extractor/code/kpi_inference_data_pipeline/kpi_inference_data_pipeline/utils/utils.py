...
def aggregate_csvs(annotation_folder):
    ...
    for f in csvs:
        ...
        cols = df.columns
        assert all([e in cols for e in config.COLUMNS_TO_READ]), "{} doesn't have certain columns {}".format(
            f, config.COLUMNS_TO_READ
        )
...
def clean_annotation(df, save_path, exclude=["CEZ"]):
    ...
    df = df.dropna(axis=0, how="any", subset=["company", "source_file", "source_page", "kpi_id", "year"]).reset_index(
        drop=True
    )
    ...
    if len(invalid_source_page) != 0:
        logger.warning(
            "Has invalid source_page format: {} and {} such examples".format(
                df["source_page"][temp.isna()].unique(), df["source_page"][temp.isna()].shape[0]
            )
        )
