...
class ProjectPaths(BaseSettings):
    ...
    path_source_pdf: Path = Field(default=Path('input/pdfs/training'))
    path_source_annotation: Path = Field(default=Path('input/annotations'))
    path_source_mapping: Path = Field(default=Path('input/kpi_mapping'))
    path_destination_pdf: Path = Field(default=Path('interim/pdfs'))
    path_destination_annotation: Path = Field(default=Path('interim/ml/annotations'))
    path_destination_mapping: Path = Field(default=Path('interim/kpi_mapping'))
    path_destination_extraction: Path = Field(default=Path('interim/ml/extraction'))
    path_destination_curation: Path = Field(default=Path('interim/ml/curation'))
    path_destination_training: Path = Field(default=Path('interim/ml/training'))
    path_destination_saved_models_relevance: Path = Field(default=Path())
    path_destination_saved_models_inference: Path = Field(default=Path())
    ...
    def path_project_data_folder(self, path_new_project_data_folder: Path) -> None:
        list_all_paths_in_model: list[str] = list(ProjectPaths.model_fields.keys())
