...
from utils.core_utils import XlsToCsvConverter, download_data_from_s3_main_bucket_to_local_folder_if_required,\
...
class Router:
    def __init__(self, main_settings: MainSettings, s3_settings: S3Settings,
        ...
        self._destination_annotation = destination_annotation
        self._s3_controller_main = s3c_main
        self._s3_controller_interim = s3c_interim
        self._extraction_server_address = None
        self._inference_server_address = None
    ...
    def _convert_xls_to_csv(self):
        download_data_from_s3_main_bucket_to_local_folder_if_required(self.s3_controller_main, self.source_annotation, Path(self.s3_settings.prefix) / Path('input/annotations'))
        self.converter.convert()
        upload_data_from_local_folder_to_s3_interim_bucket_if_required(self.s3_controller_interim, self.destination_annotationn, Path(self.s3_settings.prefix) / Path('interim/ml/annotations'))
    ...
    def _check_for_train_relevance_training_and_send_request(self):
        ...
        if self._main_settings.train_relevance.train:
            self.self._send_payload_to_server_address_with_node(self._inference_server_address, 'train_relevance')
        else:
    ...
    def _check_for_generate_text_3434(self):
        try:
            temp = generate_text_3434(self._main_settings.general.project_name, 
