    ...
    try:
        ...
        if s3_usage:
            ...
            s3c_interim.download_files_in_prefix_to_dir(
                project_prefix_data + "/interim/ml/curation", os.path.dirname(file_config.curated_data)
            )
        ...
        output_model_folder = os.path.join(
            ...
            file_config.experiment_type,
            file_config.data_type,
            file_config.output_model_name,
        )
        ...
        if relevance_training_settings["input_model_name"] is not None:
            ...
            if args["s3_usage"]:
                # Download model
                model_rel_prefix = str(
                    pathlib.Path(s3_settings["prefix"]) / project_name / "models" / "RELEVANCE" / "Text"
                )
                output_model_zip = model_dir + ".zip"
                s3c_main.download_file_from_s3(
                    output_model_zip, model_rel_prefix, relevance_training_settings["input_model_name"] + ".zip"
                )
        ...
        train_config.run_cv = relevance_training_settings["training"][
            "run_cv"
        ]  # running cross-validation won't save a model
        ...
        name_out = os.path.join(
            str(MODEL_FOLDER), project_name, "result_rel_" + relevance_training_settings["output_model_name"] + ".json"
        )
        with open(name_out, "w") as f:
            json.dump(result, f)

        if s3_usage:
            train_rel_prefix = os.path.join(
                project_prefix_project_models, file_config.experiment_type, file_config.data_type
            )
            output_model_zip = os.path.join(
                str(MODEL_FOLDER),
                file_config.experiment_name,
                file_config.experiment_type,
                file_config.data_type,
                file_config.output_model_name + ".zip",
            )
            with zipfile.ZipFile(output_model_zip, "w", zipfile.ZIP_DEFLATED) as zipf:
                zipdir(output_model_folder, zipf)
            response = s3c_main.upload_file_to_s3(
                filepath=output_model_zip, s3_prefix=train_rel_prefix, s3_key=file_config.output_model_name + ".zip"
            )
            response_2 = s3c_main.upload_file_to_s3(
                filepath=name_out,
...
def run_infer_relevance():
    ...
    BASE_OUTPUT_FOLDER = (
        BASE_DATA_PROJECT_FOLDER / "output" / relevance_infer_config.experiment_type / relevance_infer_config.data_type
    )
    ...
    output_model_folder = os.path.join(
        str(MODEL_FOLDER), project_name, relevance_infer_config.experiment_type, relevance_infer_config.data_type
    )
    ...
    if s3_usage:
        ...
        # Download model
        train_rel_prefix = os.path.join(
            project_prefix_project_models, relevance_infer_config.experiment_type, relevance_infer_config.data_type
        )
        output_model_zip = os.path.join(output_model_folder, args["train_relevance"]["output_model_name"] + ".zip")
        s3c_main.download_file_from_s3(
            output_model_zip, train_rel_prefix, args["train_relevance"]["output_model_name"] + ".zip"
        )
        with zipfile.ZipFile(output_model_zip, "r") as zip_ref:
            zip_ref.extractall(output_model_folder)
        os.remove(output_model_zip)
        # Download extraction files
        s3c_interim.download_files_in_prefix_to_dir(project_prefix_data + "/interim/ml/extraction", EXTRACTION_FOLDER)
        # Download annotation files
        s3c_interim.download_files_in_prefix_to_dir(project_prefix_data + "/interim/ml/annotations", ANNOTATION_FOLDER)
    ...
    if s3_usage:
        project_prefix_project_output = (
            pathlib.Path(s3_settings["prefix"] + "/" + project_name + "/data/output")
            / relevance_infer_config.experiment_type
...
def run_train_kpi():
    ...
    try:
        ...
        if args["s3_usage"]:
            ...
            s3c_interim.download_file_from_s3(
                filepath=tkpi.agg_annotation,
            ...
            s3c_main.download_file_from_s3(
                ...
                s3_prefix=str(project_prefix_data / "input" / "kpi_mapping"),
                s3_key="kpi_mapping.csv",
            )

            # Download text_3434 file
            s3c_interim.download_file_from_s3(
                tkpi.relevant_text_path, s3_prefix=str(project_prefix_data / "interim" / "ml"), s3_key="text_3434.csv"
            )

            # Download extractions
            s3c_interim.download_files_in_prefix_to_dir(
                str(project_prefix_data / "interim" / "ml" / "extraction"),
                str(config.TextKPIInferenceCurator_kwargs["extracted_text_json_folder"]),
            )

        _, _ = tkpi.curate(
            curation_input["val_ratio"],
            curation_input["seed"],
            curation_input["find_new_answerable"],
            curation_input["create_unanswerable"],
        )
        ...
        if kpi_inference_training_settings["input_model_name"] is not None:
            ...
            if args["s3_usage"]:
                # Download model
                model_inf_prefix = str(
                    pathlib.Path(s3_settings["prefix"]) / project_name / "models" / "KPI_EXTRACTION" / "Text"
                )
                output_model_zip = model_dir + ".zip"
                s3c_main.download_file_from_s3(
                    output_model_zip, model_inf_prefix, kpi_inference_training_settings["input_model_name"] + ".zip"
                )
                with zipfile.ZipFile(output_model_zip, "r") as zip_ref:
                    zip_ref.extractall(base_model_dir)
                os.remove(output_model_zip)
        ...
        name_out = os.path.join(
            str(MODEL_FOLDER),
            project_name,
            "result_kpi_" + kpi_inference_training_settings["output_model_name"] + ".json",
        )
        with open(name_out, "w") as f:
            json.dump(result, f)
        ...
        if args["s3_usage"]:
            ...
            train_inf_prefix = str(
                pathlib.Path(s3_settings["prefix"]) / project_name / "models" / "KPI_EXTRACTION" / "Text"
            )
            output_model_folder = str(
                pathlib.Path(MODEL_FOLDER)
                / project_name
                / "KPI_EXTRACTION"
                / "Text"
                / kpi_inference_training_settings["output_model_name"]
            )
            ...
            response = s3c_main.upload_file_to_s3(
                filepath=output_model_folder + ".zip",
            ...
            s3c_interim.upload_files_in_dir_to_prefix(
                source_dir=str(DATA_FOLDER / project_name / "interim" / "ml" / "training"),
                s3_prefix=str(
                    pathlib.Path(s3_settings["prefix"]) / project_name / "data" / "interim" / "ml" / "training"
                ),
            )
            create_directory(output_model_folder)
            create_directory(output_model_folder)
...
def run_infer_kpi():
    ...
    if s3_usage:
        ...
        s3c_main.download_file_from_s3(
            output_model_zip, train_inf_prefix, args["train_kpi"]["output_model_name"] + ".zip"
        )
        with zipfile.ZipFile(output_model_zip, "r") as zip_ref:
            zip_ref.extractall(output_model_folder)
        os.remove(output_model_zip)
        ...
        for data_type in qa_infer_config.data_types:
            ...
            if s3_usage:
                ...
                s3c_main.download_files_in_prefix_to_dir(
                    str(project_prefix_output / "RELEVANCE" / data_type), relevance_result_dir
                )
            ...
            if s3_usage:
                ...
                s3c_main.upload_files_in_dir_to_prefix(
                    output_results_folder, str(project_prefix_output / "KPI_EXTRACTION" / "ml" / data_type)
                )
                create_directory(relevance_result_dir)
...
if __name__ == "__main__":
    ...
    # Add the arguments
    parser.add_argument("--port", type=int, default=6000, help="port to use for the infer server")
