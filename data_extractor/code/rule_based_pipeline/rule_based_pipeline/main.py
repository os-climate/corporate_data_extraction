...
def main():
    ...
    kpis = (
        test_prepare_kpispecs()
    )  # TODO: In the future, KPI specs should be loaded from "nicer" implemented source, e.g., JSON file definiton
    ...
    for pdf in pdfs:
        ...
        cur_kpiresults = analyze_pdf(
            config.global_raw_pdf_folder + pdf,
            kpis,
            DEFAULT_YEAR,
            info_file_contents,
            wildcard_restrict_page="*",
            assume_conversion_done=False,
            force_parse_pdf=False,
        )  ### TODO:  Modify * in order to analyze specfic page, e.g.:  *00042 ###
    ...
    print_verbose(
        1, "Total run-time: " + str(total_time) + " sec ( " + str(total_time / max(len(pdfs), 1)) + " sec per PDF)"
    )
