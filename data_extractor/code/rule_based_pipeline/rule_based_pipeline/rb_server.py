...
def run_rb(project_name, verbosity, s3_usage, s3_settings):
    ...
    if s3_usage:
        ...
        s3c_main.download_files_in_prefix_to_dir(project_prefix + "/input/pdfs/inference", raw_pdf_folder)
    run_rb_int(raw_pdf_folder, working_folder, output_folder, verbosity)
    if s3_usage:
        s3c_main.upload_files_in_dir_to_prefix(output_folder, project_prefix + "/output/KPI_EXTRACTION/rb")
    return True
...
def run_xy_ml():
    ...
    if args["s3_usage"]:
        ...
        s3c_main.download_file_from_s3(
            csv_path, project_prefix + "/output/KPI_EXTRACTION/joined_ml_rb", args["csv_name"]
        )
    cmd = (
        "python3 /app/code/rule_based_pipeline/rule_based_pipeline/main_find_xy.py"
        + ' --raw_pdf_folder "'
        + raw_pdf_folder
        + '"'
        + ' --working_folder "'
    ...
    if args["s3_usage"]:
        s3c_main.upload_file_to_s3(
            filepath=csv_path, s3_prefix=project_prefix + "/output/KPI_EXTRACTION/joined_ml_rb", s3_key=args["csv_name"]
        )
    return Response(response={}, status=200)
...
if __name__ == "__main__":
    ...
    # Add the arguments
    parser.add_argument("--port", type=int, default=8000, help="port to use for the infer server")
