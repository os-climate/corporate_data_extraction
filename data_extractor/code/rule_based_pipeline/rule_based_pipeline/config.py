...
global_verbosity = (
    1  ### TODO: Change verbosity here. verbosity 6-8 are good values for debugging without too much output ###
)
...
global_be_more_generous_with_good_tables = (
    True  # default=False. If true, we will consider some tables as good that normally considered bad
)

global_table_merge_non_overlapping_rows = (
    True  # default: Fale. If true, system will try to merge non-overlapping rows that probably belong to the same cell
)
...
global_ignore_all_years = (
    False  # default: False. Set it to true to ignore all years for every KPI (this is used for CDP reports)
)
