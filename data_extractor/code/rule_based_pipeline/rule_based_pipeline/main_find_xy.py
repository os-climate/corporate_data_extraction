        ...
        if p.page_num == pageNum:
            for i in p.items:
                ...
                try:
                    ...
                    wordIndex = len(
                        contxt[:index].strip().split()
                    )  # get str before substring's 1st letter, split it by word，length = index of substring's 1st word
                    print_verbose(2, "wordIndex:")
                    print_verbose(2, wordIndex)
                    # print_verbose(2, "input txt:" + txt)
                    res.extend(
                        (
                            i.words[wordIndex].rect.get_coordinates()[0] / p.page_width,
...
def modify_csv(csv, info_file_contents):
    ...
    for c in range(len(csvPD)):  # check columns
        ...
        if str(csvPD["POS_X"][c]) == "nan" or str(csvPD["POS_Y"][c]) == "nan":
            coordis = analyze_pdf(
                config.global_raw_pdf_folder + str(csvPD["PDF_NAME"][c]),
                int(csvPD["PAGE"][c]),
                str(csvPD["ANSWER_RAW"][c]),
                info_file_contents,
                assume_conversion_done=False,
                force_parse_pdf=False,
            )
...
def main():
    ...
    parser.add_argument("--raw_pdf_folder", type=str, default=None, help="Folder where PDFs are stored")
    parser.add_argument("--working_folder", type=str, default=None, help="Folder where working files are stored")
    parser.add_argument("--pdf_name", type=str, default=None, help="name of pdf which you want to check")
    ...
    parser.add_argument("--output_folder", type=str, default=None, help="Folder where output is stored")
    parser.add_argument("--verbosity", type=int, default=1, help="Verbosity level (0=shut up)")

    args = parser.parse_args()
    config.global_raw_pdf_folder = (
        remove_trailing_slash(get_input_variable(args.raw_pdf_folder, "What is the raw pdf folder?")).replace("\\", "/")
        + r"/"
    )
    config.global_working_folder = (
        remove_trailing_slash(get_input_variable(args.working_folder, "What is the working folder?")).replace("\\", "/")
        + r"/"
    )
    ...
    config.global_output_folder = (
        remove_trailing_slash(get_input_variable(args.output_folder, "What is the output folder?")).replace("\\", "/")
        + r"/"
    )
