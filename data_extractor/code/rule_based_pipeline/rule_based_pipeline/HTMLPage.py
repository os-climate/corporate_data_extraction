...
class HTMLPage:
    ...
    def detect_split_items(self):
        ...
        def find_space_in_direction(all_words, k0, dir):
            ...
            if dir == -1:
                return (
                    self.items[item_id].words[word_id].rect.x0 - self.items[item_id].words[word_id - 1].rect.x1
                    if word_id > 0
                    else 9999999
                )
            elif dir == 1:
                return (
                    self.items[item_id].words[word_id + 1].rect.x0 - self.items[item_id].words[word_id].rect.x1
        ...
        for k in range(len(all_words)):
            ...
            isnum_l = (
                Format_Analyzer.looks_weak_numeric(self.items[item_id].words[word_id - 1].txt) if word_id > 0 else False
            )
            isnum_r = (
                Format_Analyzer.looks_weak_numeric(self.items[item_id].words[word_id + 1].txt)
                if word_id < num_words - 1
                else False
            )
            ...
            if (
                len(right_aligned_words) > threshold_rows_r
                and word_id < num_words - 1
                and space_width_r > threshold_space_r
            ):
                # print(self.items[item_id].words[word_id].txt)
                # print('!!!')
                do_split_r.append(k)
        ...
        for ij in all_words:
            ...
            if (
                self.items[ij[0]].words[ij[1]].rect.x0 - self.items[ij[0]].words[ij[1] - 1].rect.x1
                < self.items[item_id].space_width * 0
            ):
                continue  # words are too close to split

            if (
                self.items[ij[0]].words[ij[1]].rect.x0 - self.items[ij[0]].words[ij[1] - 1].rect.x1
                < self.items[item_id].space_width * 1.5
                and not Format_Analyzer.looks_weak_numeric(self.items[ij[0]].words[ij[1]].txt)
                and not Format_Analyzer.looks_weak_numeric(self.items[ij[0]].words[ij[1] - 1].txt)
            ):
                continue  # words are too close to split
            ...
            print_verbose(
                ...
                + str(ij[1])
                + "(x1="
                + str(self.items[ij[0]].words[ij[1] - 1].rect.x1)
                + "<-> x0="
                + str(self.items[ij[0]].words[ij[1]].rect.x0)
                + " , space_width= "
                + str(self.items[item_id].space_width),
            )
    ...
    def find_items_within_rect_all_categories(self, rect):  # returns list of indices
        ...
        for i in range(len(self.items)):
            if (
                Rect.calc_intersection_area(self.items[i].get_rect(), rect) > self.items[i].get_rect().get_area() * 0.3
            ):  # 0.5?
                res.append(i)
        return res
    ...
    def find_items_within_rect(self, rect, categories):  # returns list of indices
        ...
        for i in range(len(self.items)):
            if self.items[i].category in categories:
                if (
                    Rect.calc_intersection_area(self.items[i].get_rect(), rect)
                    > self.items[i].get_rect().get_area() * 0.3
                ):  # 0.5?
                    res.append(i)
        return res
    ...
    def explode_item(self, idx, sep=" "):  # return concatenated txt
        def expl_int(dir, idx, sep):
            return (
                ...
                + (
                    ""
                    if self.items[idx].right_id == -1 or dir < 0
                    else sep + expl_int(1, self.items[idx].right_id, sep)
                )
            )
    ...
    def guess_all_alignments(self):
        for it in self.items:
            ...
            dummy, score_center = self.find_vertical_aligned_items(
                it, ALIGN_CENTER, DEFAULT_VTHRESHOLD
            )  # New-27.06.2022
    ...
    def identify_connected_txt_lines(self):
        ...
        for cur_x, cnt in self.left_distrib.items():
            ...
            for i in range(len(self.items)):
                if (
                    self.items[i].pos_x >= cur_x
                    and self.items[i].pos_x <= cur_x + cur_threshold
                    and self.items[i].pos_y > last_pos_y
                ):
            ...
            for i in range(len(cur_lines) - 1):
                ...
                if (
                    (next_y > cur_y + min(cur_item.height + max_allowed_spacing, 2 * cur_item.height))
                    or (cur_item.font_size != next_item.font_size)  # too far apart
                    or (cur_item.font_file != next_item.font_file)  # different font sizes
                ):  # different fonts faces
                    cur_threshold = 0
                    continue
    ...
    def mark_other_text_components(self):
        ...
        for cur_x, cnt in self.left_distrib.items():
            ...
            for i in range(len(cur_lines)):
                ...
                if prev_item_id != -1 and next_item_id != -1:
                    y_threshold = 2 * max(
                        self.items[cur_item_id].height, self.items[prev_item_id].height, self.items[next_item_id].height
                    )

                    if (
                        self.items[prev_item_id].category == CAT_RUNNING_TEXT
                        and self.items[next_item_id].category == CAT_RUNNING_TEXT
                        and abs(cur_y - prev_y) < y_threshold
                        and abs(cur_y - next_y) < y_threshold
                    ):
                ...
                if (prev_item_id == -1 or self.items[cur_item_id].prev_id == -1) and next_item_id != -1:
                    ...
                    if self.items[next_item_id].category == CAT_RUNNING_TEXT and abs(cur_y - next_y) < y_threshold:
                        if self.items[cur_item_id].height > self.items[next_item_id].height:
                            self.items[cur_item_id].category = CAT_HEADLINE
                        else:
            ...
            for i in range(len(cur_lines) - 1):
                ...
                print_verbose(
                    9, "--> mark_other_text_components \ multi-rows headline: " + str(self.items[cur_item_id])
                )
                next_item_id, next_y = cur_lines[i + 1]
                ...
                if self.items[cur_item_id].font_size <= self.items[next_item_id].font_size and not (
                    self.items[cur_item_id].font_size == self.items[next_item_id].font_size
                    and self.items[cur_item_id].is_bold
                    and not self.items[next_item_id].is_bold
                ):
                ...
                print_verbose(
                    9, "----> cur_y, next_y , y_threshold = " + str(cur_y) + "," + str(next_y) + "," + str(y_threshold)
                )

                if (
                    abs(cur_y - next_y) < y_threshold
                    and self.items[cur_item_id].height > self.items[next_item_id].height
                ):
        ...
        for it in self.items:
            ...
            for jt in self.items:
                if (
                    it == jt
                    or jt.category == CAT_RUNNING_TEXT
                    or jt.category == CAT_HEADLINE
                    or jt.category == CAT_OTHER_TEXT
                    or jt.category == CAT_FOOTER
                ):
                    continue  # we do not consider these ones (=> text is assumed to be isolated, even if any of these ones are near)
                cur_dist = Rect.raw_rect_distance(
                    it.pos_x,
                    it.pos_y,
                    it.pos_x + it.width,
                    it.pos_y + it.height,
    ...
    def find_vnearest_taken_components(
        self, initial_item
        ...
        for i in vitems:
            ...
            for j in vitems:
                if i == j:
                    continue
                if (
                    self.items[j].pos_y + self.items[j].height >= self.items[i].pos_y
                    and self.items[j].pos_y <= self.items[i].pos_y + self.items[i].height
                ):
    ...
    def discover_table_column(self, initial_item):
        ...
        vitems = self.sort_out_non_vconnected_items(
            vitems, -1 if top is None else top.pos_y, 9999999 if bottom is None else bottom.pos_y
        )
    ...
    def discover_subtables_recursively(
        self, initial_item, step
    ):  # step=0 => discover col; step=1 => discover row. each subtable is a column
    ...
    def discover_table(self, initial_item):
        ...
        while not done:
            ...
            if table.is_good_table():
                ...
                missing_items = self.find_items_within_rect(
                    table.table_rect, [CAT_HEADLINE, CAT_OTHER_TEXT, CAT_RUNNING_TEXT, CAT_FOOTER]
                )
    ...
    def render_to_png(self, in_dir, out_dir):
        base = (
            Image.open(in_dir + r"/page" + str(self.page_num) + ".png")
            .convert("RGBA")
            .resize((self.page_width, self.page_height))
        )
        ...
        if not RENDERING_USE_CLUSTER_COLORS:
            ...
            for t in self.tables:
                context.rectangle(
                    [(t.table_rect.x0, t.table_rect.y0), (t.table_rect.x1, t.table_rect.y1)],
                    fill=table_bg_color,
                    outline=(0, 0, 0, 255),
                )
                first = True
        ...
        for it in self.items:
            if it.next_id != -1 and it.category == CAT_RUNNING_TEXT:
                line_shape = [
                    (it.pos_x + 5, int(it.pos_y + it.height / 2)),
                    (
                        self.items[it.next_id].pos_x + 5,
                        self.items[it.next_id].pos_y + int(self.items[it.next_id].height / 2),
                    ),
                ]
        ...
        for i in range(len(self.items) - 1):
            if keep[i]:
                for j in range(i + 1, len(self.items)):
                    if Rect.calc_intersection_area(self.items[i].get_rect(), self.items[j].get_rect()) > 0.0:
                        # overlapping items => remove it
                        keep[j] = False
                        print_verbose(
                            5,
                            "Removing item : " + str(self.items[j]) + ", because overlap with : " + str(self.items[i]),
                        )
        ...
        if detected_strange_char > 10:
            ...
            for i in range(len(new_bytes)):
                if i < len(new_bytes) - 6:
                    ...
                    if (
                        new_bytes[i] == 60
                        and new_bytes[i + 1] == 98
                        and new_bytes[i + 2] == 111
                        and new_bytes[i + 3] == 100
                        and new_bytes[i + 4] == 121
                        and new_bytes[i + 5] == 62
                    ):
                        hit_body = True
    ...
    def parse_html_file(fonts_dir, htmlfile):
        ...
        for i in range(0, len(html_file)):
            ...
            if pattern_div.match(h):
                ...
                for s in spans:
                    if pattern_span.match(s):
                        ...
                        if int(gs[0]) in font_dict and font_dict[int(gs[0])] in font_url_dict:
                            span_font = ImageFont.truetype(
                                fonts_dir + "/" + font_url_dict[font_dict[int(gs[0])]], int(gs[1])
                            )
                        ...
                        try:
                            space_width = max(
                                space_width, get_text_width(" ", span_font), get_text_width("x", span_font)
                            )
                        except:
                            span_font = ImageFont.truetype(config.global_approx_font_name, int(gs[1]))
                            item.font_file = config.global_approx_font_name
                ...
                item.space_width = max(
                    space_width, get_text_width(" ", ImageFont.truetype(config.global_approx_font_name, item.font_size))
                )
