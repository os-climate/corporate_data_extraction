...
class Format_Analyzer:
    ...
    pattern_year = re.compile(
        r"^[^0-9]*(19[8-9][0-9](/[0-9][0-9])?|20[0-9][0-9](/[0-9][0-9])?)[^0-9]*$"
    )  # 1980-2099 #by Lei
    pattern_year_extended_1 = re.compile(
        r"^.*[0-3][0-9](\.|\\|/)[0-3][0-9](\.|\\|/)(19[8-9][0-9]|20[0-9][0-9]).*$"
    )  # 1980-2099
    pattern_year_extended_2 = re.compile(
        r"^.*(19[8-9][0-9]|20[0-9][0-9])(\.|\\|/)[0-3][0-9](\.|\\|/)[0-3][0-9].*$"
    )  # 1980-2099
    ...
    def looks_pagenum(val):
        return (
            Format_Analyzer.pattern_pagenum.match(val.replace(" ", "")) and len(val) > 0 and val.replace(" ", "") != "0"
        )
    ...
    def looks_running_text(val):
        ...
        return (
            ((num_full_stops > 2) or (num_full_stops > 1 and num_comma > 1)) and (num_letters > 30) and (num_space > 10)
        )
