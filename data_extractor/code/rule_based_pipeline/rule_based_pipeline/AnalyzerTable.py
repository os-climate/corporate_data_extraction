...
class AnalyzerTable:
    ...
    def get_depth(self, i, j, dir):
        ident_threshold = (
            (3.0 / 609.0) * self.htmlpage.page_width
            if dir == HIERARCHY_DIR_UP
            else (3.0 / 609.0) * self.htmlpage.page_height
        )
    ...
    def get_first_non_empty_row(
        self, r0, c0
    ):  # typically, set r0 = 0 to search from top. If no such row found, then "num_rows" is returned
        r = r0
    ...
    def find_applicable_year_line(self, r0):
        ...
        if first_match is not None:
            ...
            for i in range(len(self.year_rows) - 1, -1, -1):
                if (
                    self.year_rows[i].row_num < first_match.row_num
                    and self.year_rows[i].row_num >= first_match.row_num - 7
                ):  # max 7 lines above
    ...
    def find_applicable_unit_item(self, kpispecs, r0):
        ...
        items_idx = self.htmlpage.find_items_within_rect(
            search_rect,
            [CAT_HEADLINE, CAT_OTHER_TEXT, CAT_TABLE_DATA, CAT_TABLE_HEADLINE, CAT_TABLE_SPECIAL, CAT_MISC, CAT_FOOTER],
        )
        match_idx = -1
        for i in items_idx:
        ...
        for i in items_idx:
            ...
            if cur_dist < best_dist:
                for w in it.words:
                    ...
                    print_verbose(
                        11, '..................... Analyzing possible year string: "' + w.txt + '" => ' + str(cur_year)
                    )
    ...
    def find_kpi_with_direct_years(self, kpispecs, bonus):
        ...
        for i in range(self.get_num_rows()):
            ...
            txt_nodes = txt_nodes + (
                [previous_txt_node_with_no_values]
                if previous_txt_node_with_no_values != "" and previous_txt_node_with_no_values not in txt_nodes
                else []
            )
            ...
            for y, it in value_items.items():
                ...
                if total_score < kpispecs.minimum_score:
                    print_verbose(
                        ...
                        "---> Total score "
                        + str(total_score)
                        + " is less than min. score "
                        + str(kpispecs.minimum_score),
                    )
                    continue
                ...
                print_verbose(
                    ...
                    + ",h_score="
                    + str(h_score)
                    + ",anywhere_match_score="
                    + str(anywhere_match_score)
                    + ",bonus="
                    + str(bonus)
                    + ", multiplier="
                    + str(multiplier),
    ...
    def find_kpi_with_indirect_years(self, kpispecs, bonus):
            ...
            for i in range(self.get_num_rows()):
                ...
                for j in range(fixed_left_column + 1, self.get_num_cols()):
                    ...
                    if kpi_year == -1:
                        ...
                        kpi_year = self.search_year_agressive(
                            search_rect,
                            self.default_year - 10,
                            self.default_year,
                            base_pos_x,
                            base_pos_y,
                            aggressive_year_pattern=False,
                        )
                        if kpi_year == -1:
        ...
        print_verbose(
            5,
            "===> found AT.indirect KPIs on Page "
            + str(self.htmlpage.page_num)
            + ": "
            + str(res)
            + "\n================================",
        )
