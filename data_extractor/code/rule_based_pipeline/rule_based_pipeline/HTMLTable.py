...
class HTMLTable:
    ...
    def count_regular_items_in_rect(self, rect):
        ...
        for it in self.items:
            if (
                it.category in [CAT_HEADLINE, CAT_OTHER_TEXT, CAT_RUNNING_TEXT, CAT_FOOTER]
                and Rect.calc_intersection_area(it.get_rect(), rect) / it.get_rect().get_area() > 0.1
            ):
                res += 1
        return res
    ...
    def is_row_mergable(self, r0):  # return True iff row r0 and r0+1 can be merged
        ...
        for j in range(self.num_cols):
            if (
                self.has_item_at(r0, j)
                and self.has_item_at(r0 + 1, j)
                and not self.get_item(r0, j).is_mergable(self.get_item(r0 + 1, j))
            ):
                return False
        ...
        while c0 < self.num_cols and c1 < self.num_cols:
            if self.has_item_at(r0, c0) and self.has_item_at(r1, c1):
                ...
                if min(x1_0, x1_1) - max(x0_0, x0_1) >= 0:
                    ...
                    print_verbose(
                        7, "These are the items " + str(self.get_item(r0, c0)) + ", and " + str(self.get_item(r1, c1))
                    )
    ...
    def throw_away_non_connected_rows(self):  # throw away rows that are probably not connected to the table
        def is_connected_row(r0):
            ...
            y_limit = (
                min_y0 + (max_y0 - min_y0) * config.global_row_connection_threshold
            )  # at least space that woulld occupy 4x row r0 are (not) empty #New: 27.06.2022 (was previously: * 4)
        ...
        for i in range(self.num_rows):
            ...
            if num_rows_with_left_txt > 2 and num_numeric_rows > 2 and cur_delta_y > last_delta_y * 1.05 + 2:
                if cur_numeric_values == 0 and cur_header_values > 0 and cur_other_values == 0:
                    print_verbose(
                        5,
                        "Throw away non-connected rows after probably new headline at row = "
                        + str(i)
                        + ", cur/last_delta_y="
                        + str(cur_delta_y)
                        + "/"
            ...
            for i in range(self.num_rows):
                ...
                for it in self.items:
                    ...
                    if Rect.calc_intersection_area(cur_rect, it_rect) > 0:
                        print_verbose(
                            2, "----->> With " + str(self.get_item(i, c0)) + " the item " + str(it) + " overlaps"
                        )
                        num_reg_text += 1
    ...
    def throw_away_cols_at_next_paragraph(self, paragraphs):
        ...
        for p in paragraphs:
            ...
            for it in self.items:
                if (
                    it.pos_x == p
                    and it.category in (CAT_RUNNING_TEXT, CAT_HEADLINE)
                    and it.pos_x < self.table_rect.x1
                    and it.pos_x + it.width > self.table_rect.x0
                ):
                    if (
                        it.pos_y < self.table_rect.y1 and it.pos_y + it.height * 5 > self.table_rect.y0
                    ) or (  # TODO 5 was 10, test it
                        it.pos_y > self.table_rect.y0 and it.pos_y - it.height * 5 < self.table_rect.y1
                    ):  # TODO 5 was 10, test it
                        cnt += 1
                        # print(it)
            if cnt > 3:
        ...
        for j in range(self.num_cols):
            ...
            print_verbose(
                ...
                + "/"
                + str(self.cols[j].x1)
                + " belong to paragraph p_idx = "
                + str(cur_para_idx)
                + ", which is at "
                + str(my_paragraphs[cur_para_idx] if cur_para_idx != -1 else None)
                + " px",
            )
    ...
    def throw_away_cols_after_year_list(self):  # throw away cols after a list of years is over
        ...
        for i in range(self.num_rows):
            ...
            while j < self.num_cols - 1:
                if (
                    self.has_item_at(i, j)
                    and self.has_item_at(i, j + 1)
                    and Format_Analyzer.looks_year(self.get_item(i, j).txt)
                    and Format_Analyzer.looks_year(self.get_item(i, j + 1).txt)
                    and abs(
                        Format_Analyzer.to_year(self.get_item(i, j + 1).txt)
                        - Format_Analyzer.to_year(self.get_item(i, j).txt)
                    )
                    == 1
                ):
                    ...
                    for j1 in range(j + 1, self.num_cols):
                        if (
                            self.has_item_at(i, j1)
                            and Format_Analyzer.looks_year(self.get_item(i, j1).txt)
                            and Format_Analyzer.to_year(self.get_item(i, j1).txt)
                            - Format_Analyzer.to_year(self.get_item(i, j1 - 1).txt)
                            == dir
                        ):
                            cur_year_cols.c1 = j1
                        else:
    ...
    def throw_away_duplicate_looking_cols(
        ...
        def are_cols_similar(c0, c1):
            return (
                self.col_looks_like_text_col(c0)
                and self.col_looks_like_text_col(c1)
                and Format_Analyzer.cnt_overlapping_items(self.get_all_cols_as_text(c0), self.get_all_cols_as_text(c1))
                > 3
            )
    ...
    def identify_non_numeric_special_items(self):
        ...
        for j in range(self.num_cols):
            if col_looks_numeric(j):
                ...
                if (
                    self.has_item_at(r0, j)
                    and is_only_item_in_row(r0, j)
                    and (
                        Format_Analyzer.looks_words(self.get_item(r0, j).txt)
                        or Format_Analyzer.looks_other_special_item(self.get_item(r0, j).txt)
                    )
                ):
    ...
    def identify_overlapping_special_items(self):
        ...
        def calc_single_col_boundary(tmp_idx, col0):
            ...
            for i in range(self.num_rows):
                ...
                if cur_idx != -1:
                    cur_bdry = (
                        min(cur_bdry[0], self.items[cur_idx].pos_x),
        ...
        print_verbose(
            3,
            "---> find_allowed_set_rec completed after time="
            + str(t_end - t_start)
            + "sec,  recursions="
            + str(rec_counter),
        )
    ...
    def throw_away_distant_special_items(self, page_width):
        ...
        for i in self.special_idx:
            ...
            if dist <= DEFAULT_SPECIAL_ITEM_MAX_DIST and self.items[i].pos_y <= self.table_rect.y1:
                ...
                print_verbose(
                    5,
                    "Throw away special item: "
                    + str(self.items[i])
                    + " from table with rect : "
                    + str(self.table_rect)
                    + " and distance: "
                    + str(dist),
                )
    ...
    def cleanup_table(self, page_width, paragraphs):
        ...
        while True:
            ...
            if (
                cur_num_actual_items == old_num_actual_items
                and self.num_rows == old_num_rows
                and self.num_cols == old_num_cols
            ):
    ...
    def is_good_table(self):
        ...
        return (
            (cnt_numerics > 3 and density > 0.6)
            or (cnt_numerics > 7 and density > 0.4)
            or cnt_numerics > 10
            ...
            or (
                ...
                and num_items > 2
                and density > 0.4
                and config.global_be_more_generous_with_good_tables
            )
        )
    ...
    def merge(tab1, tab2, page_width):  # note: tables must belong to same HTMLPage !
        def cols_are_mergable(tab1, tab2, col1, col2):
            ...
            for i in range(tab1.num_rows):
                ...
                c1_x1 = max(
                    c1_x1,
                    tab1.get_item(i, col1).pos_x + tab1.get_item(i, col1).width if tab1.has_item_at(i, col1) else -1,
                )
            ...
            for i in range(tab2.num_rows):
                ...
                c2_x1 = max(
                    c2_x1,
                    tab2.get_item(i, col2).pos_x + tab2.get_item(i, col2).width if tab2.has_item_at(i, col2) else -1,
                )
        ...
        def find_all_new_columns(tab1, tab2, tmp_rows, threshold_px):
            ...
            while tab1_col < tab1.num_cols or tab2_col < tab2.num_cols:
                ...
                if (
                    use_tab1 and use_tab2 and not cols_are_mergable(tab1, tab2, tab1_col, tab2_col)
                ):  # TODO: and cols do not intersect by any rect
                    ...
                    if len(list_idx) > 1:
                        ...
                        for k in range(len(list_idx) - 1):
                            ...
                            if not it.is_mergable(it1):
                                ...
                                print_verbose(
                                    5, "-----> Split neccessary: " + str(it) + " cant be merged with " + str(it1)
                                )
                                print_verbose(
                                    5,
                                    "-----> Split is here: "
    ...
    def is_non_overlapping_row_mergable(self, r0):  # return True iff row r0 and r0+1 can be merged
        ...
        if n0 and n1:
            print_verbose(
                8, "--->> is_non_overlapping_row_mergable: Rows r0=" + str(r0) + " and r0+1 have both numbers"
            )
        ...
        for j in range(self.num_cols):
            ...
            if both_filled and not self.get_item(r0, j).is_weakly_mergable_after_reconnect(self.get_item(r0 + 1, j)):
                print_verbose(
                    8,
                    "--->> is_non_overlapping_row_mergable: Rows r0="
                    + str(r0)
                    + " and r0+1 are not mergable. Items:"
                    + str(self.get_item(r0, j))
                    + " and "
                    + str(self.get_item(r0 + 1, j)),
                )
                return False, 0
            if both_filled:
                ...
                if (
                    self.get_item(r0, j).pos_y + self.get_item(r0, j).height * (3.0 if n0 else 2.1)
                    < self.get_item(r0 + 1, j).pos_y
                ):
                    print_verbose(
                        8,
                        "--->> is_non_overlapping_row_mergable: Rows r0="
                        + str(r0)
                        + " and r0+1 are too far apart. Items:"
                        + str(self.get_item(r0, j))
                        + " and "
                        + str(self.get_item(r0 + 1, j)),
                    )
        ...
        if not has_mergable_candidates:
            print_verbose(
                8, "--->> is_non_overlapping_row_mergable: Rows r0=" + str(r0) + " and r0+1 have no mergable candidates"
            )
            return False, 0
        ...
        for j in range(self.num_cols):
            if (self.has_item_at(r0, j) and self.get_item(r0, j).get_font_characteristics() != font_chars) or (
                self.has_item_at(r0 + 1, j) and self.get_item(r0 + 1, j).get_font_characteristics() != font_chars
            ):
                print_verbose(
                    8,
                    "--->> is_non_overlapping_row_mergable: Rows r0=" + str(r0) + " and r0+1 have different font chars",
                )
                return False, 0
