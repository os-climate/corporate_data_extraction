...
class HTMLItem:
    ...
    category = None
    temp_assignment = (
        None  # an integer, that is normally set to 0. It has greater values while table extraction is in progress
    )
    ...
    def is_text_component(self):
        return (
            self.category == CAT_HEADLINE
            or self.category == CAT_OTHER_TEXT
            or self.category == CAT_RUNNING_TEXT
            or self.category == CAT_FOOTER
        )
    ...
    def is_mergable(self, it):
        ...
        if self.prev_id == -1 and it.prev_id == -1:
            return False
        return (
            (self.next_id == it.this_id or self.prev_id == it.this_id)
    ...
    def __repr__(self):
        return (
            ...
            + str(self.height)
            + ", init_height="
            + str(self.get_initial_height())
            + ", align="
            + ("L" if self.alignment == ALIGN_LEFT else "R" if self.alignment == ALIGN_RIGHT else "C")
            + ", brightness="
            + str(self.brightness)
            + (
                ", cat=" + str(self.category) + ", tmp_ass=" + str(self.temp_assignment)
                if config.global_verbosity >= 8
                else ""
            )
            + ", depth="
            + str(self.get_depth())
