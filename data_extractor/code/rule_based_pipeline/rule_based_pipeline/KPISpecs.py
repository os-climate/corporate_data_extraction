...
MATCHING_MUST_INCLUDE_EACH_NODE = (
    4  # must be includd in each node, otherwise no match. Note: this is even more strict then MATCHING_MUST_INCLUDE
)
...
DISTANCE_MOD_EUCLID_UP_LEFT = (
    2  # like 1, but we prefer looking upwards and to the left, which conforms to the typical strucutre of a table
)
...
class KPISpecs:
    ...
    class DescRegExMatch:  # regex matcher for description of KPI
        ...
        count_if_matched = (
            None  # if this is TRUE (default), then a match will be counted, in cases where a single match is needed
        )
        allow_matching_against_concat_txt = (
            None  # if this is TRUE, then we will try to match against a concatenation of all nodes. Default: FALSE
        )
        ...
        def __init__(
            ...
            self.pattern_regex = re.compile(
                pattern_raw
            )  # if case_sensitive else pattern_raw.lower()) # Note: using lower-case here would destory regexp patterns like \S
        ...
        def match_single_node(self, txt):
            return (
                True if self.pattern_regex.match(txt if self.case_sensitive else txt.lower()) else False
            )  # b/c regexp matcher returns not just a boolean value
        ...
        def match_nodes(
            ...
            if self.allow_matching_against_concat_txt:
                ...
                if self.match_single_node(concat_txt):
                    concat_final_score = self.score * (
                        self.letter_decay
                        ** max(len(Format_Analyzer.cleanup_text(concat_txt)) - self.letter_decay_disregard, 0)
                    )
                    concat_match = True
            ...
            for i in range(len(txt_nodes)):
                if self.match_single_node(txt_nodes[i]):
                    ...
                    final_score += (
                        self.score
                        * (self.score_decay**i)
                        * (self.multi_match_decay**num_hits)
                        * (
                            self.letter_decay
                            ** max(len(Format_Analyzer.cleanup_text(txt_nodes[i])) - self.letter_decay_disregard, 0)
                        )
                    )
                    num_hits += 1
                else:
    ...
    class GeneralRegExMatch:  # regex matcher for value or unit of KPI
        ...
        def match(self, txt):
            return (
                True if self.pattern_regex.match(txt if self.case_sensitive else txt.upper()) else False
            )  # b/c regexp matcher returns not just a boolean value
        ...
        def match(self, htmlpage, cur_item_idx):
            ...
            for i in range(len(htmlpage.items)):
                ...
                if self.general_match.match(txt):
                    ...
                    score_base = (
                        self.score
                        * (self.score_decay**dist_exp)
                        * (
                            self.letter_decay
                            ** max(len(Format_Analyzer.cleanup_text(txt)) - self.letter_decay_disregard, 0)
                        )
                    )
                    print_verbose(
                        9,
            ...
            if len(matches) > 0:
                print_verbose(
                    8,
                    "AnywhereRegExMatch.match of item "
                    + str(htmlpage.items[cur_item_idx])
                    + " matches with: "
                    + str(matches),
                )
    ...
    def match_nodes(self, desc_nodes):  # check if nodes are matched by this, and if yes return True togehter with score
        ...
        for d in self.desc_regex_match_list:
            ...
            if (
                d.matching_mode in (MATCHING_MAY_INCLUDE, MATCHING_MUST_INCLUDE, MATCHING_MUST_INCLUDE_EACH_NODE)
                and d.count_if_matched
                and match
                and score > 0
            ):
