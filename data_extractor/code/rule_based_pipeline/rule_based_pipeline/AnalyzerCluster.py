...
class AnalyzerCluster:
    ...
    def find_kpis_single_node(self, kpispecs, cluster):
            ...
            for i in range(len(idx_list)):
                ...
                if matching_fun(txt):
                    ...
                    for j in range(len(it.words)):
                        wtxt = (
                            it.words[j].txt if not exclude_years else Format_Analyzer.exclude_all_years(it.words[j].txt)
                        )
        ...
        raw_value_idx, raw_value, value_rect = find_nearest_matching_str(
            idx_list, base_point_x, base_point_y, kpispecs.match_value, not kpispecs.value_must_be_year
        )  # TODO: Maybe not always exclude years ?
        ...
        if kpispecs.has_unit():
            unit_idx, unit_str, foo = find_nearest_matching_str(
                idx_list, base_point_x, base_point_y, kpispecs.match_unit, True
            )  # TODO: Maybe not always exclude years ?
            if unit_idx is None:
        ...
        # get year
        year = -1
        year_idx, year_str, foo = find_nearest_matching_str(
            idx_list, base_point_x, base_point_y, Format_Analyzer.looks_year, False
        )
