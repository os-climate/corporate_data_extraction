...
def df_from_kpi_infer(infer_fpath, kpi_mapping_fpath):
    ...
    df = df[['pdf_name', 'kpi',	'kpi_id_y',	'answer','page','paragraph',	'source',	'score'	,'no_ans_score'	,'no_answer_score_plus_boost']]
...
def update_pdf_selection(pdf_path, df_annotations, kpi_of_interest):
    ...
    for output in outputs:
        ...
        if set(kpis_contained) == set(kpi_of_interest):
            ...
            pdf_selection = select_options + [(f'TODO ({len(set(kpis_contained))}/{len(kpi_of_interest)}) - ' + output, output)]
...
def preload():
    ...
    for output in outputs:
        ...
        if set(kpis_contained) == set(kpi_of_interest):
            ...
            select_options = select_options + [(f'TODO ({len(set(kpis_contained))}/{len(kpi_of_interest)}) - ' + output, output)]
...
dropdown = widgets.Dropdown(description="Choose rank of correct kpi:", 
                            options=select_list, 
                            style = {'description_width': 'initial'},
                            layout = widgets.Layout(width='50%'),
                            value = None)

w_dropdown_paragraph = widgets.Dropdown(description="Choose rank of correct pagraph:", 
                            options=select_list, 
                            style = {'description_width': 'initial'},
                            layout = widgets.Layout(width='50%'),
                            value = None)
